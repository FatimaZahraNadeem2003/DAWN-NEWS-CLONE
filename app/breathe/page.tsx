...
import { Linkedin, Instagram, Play, ChevronRight } from 'lucide-react';
...
const BreathePage = () => {
  ...
  useEffect(() => {
    if (videoRef.current) {
      videoRef.current.play().catch((error: any) => console.error("Video failed:", error));
    }
  }, []);

  // Data for Journalism Section
  const articles = [
    {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900">
      ...
      <section className="relative w-full h-[120vh] overflow-hidden bg-black">
        ...
        <div className="absolute top-0 w-full h-48 bg-gradient-to-b from-white via-white/30 to-transparent z-10"></div>
        <div className="absolute bottom-0 w-full h-48 bg-gradient-to-t from-white via-white/30 to-transparent z-10"></div>
      </section>
          ...
          {[
            ...
            <div key={i}>
              <div className="flex items-center gap-2 text-red-800 font-bold text-xs uppercase mb-4">
                <span className="w-2 h-2 bg-red-800 rounded-full"></span> {event.date} <div className="h-[1px] bg-gray-200 flex-grow"></div>
              </div>
          ...
          {articles.map((art, i) => (
            <div key={i} className="group cursor-pointer">
              <div className="rounded-xl overflow-hidden mb-4 aspect-[4/3]">
                <img src={art.img} alt="news" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              </div>
          ...
          {videos.map((vid, i) => (
            <div key={i} className="bg-white border border-blue-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow relative">
              <div className="relative rounded-lg overflow-hidden aspect-video bg-gray-200 mb-4 group">
                ...
                <img src={`https://img.youtube.com/vi/placeholder/0.jpg`} className="w-full h-full object-cover opacity-40" alt="thumb" />
